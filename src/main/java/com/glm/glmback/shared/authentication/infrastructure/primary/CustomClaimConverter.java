...
import com.glm.glmback.shared.memoizer.domain.Memoizers;
...
class CustomClaimConverter implements Converter<Map<String, Object>, Map<String, Object>> {
  ...
  private static final class StandardClaimAppender implements ClaimAppender {

    private final String key;

    private StandardClaimAppender(String key) {
      this.key = key;
    }
  ...
  private static final class NameClaimAppender implements ClaimAppender {
    ...
    public void append(Map<String, Object> claim, ObjectNode user) {
      ...
      if (user.has(NAME)) {
        String[] name = user.get(NAME).asString().split("\\s+", 0);
