...
class JourneesDeTravailServiceTest {
  ...
  private final JourneesDeTravailService service = new JourneesDeTravailService(journees, maintenant::get);

  @Test
  void shouldOuvrirLaJourneeALArrivee() {
    JourneeDeTravail journee = service.arrive(new ArriveeAEnregistrer(OPERATEUR_DUPONT, AUTEUR_DUPONT));
  ...
  void shouldOuvrirLaJourneeAUneHeureRegularisee() {
    ...
    JourneeDeTravail journee = service.arrive(new ArriveeAEnregistrer(OPERATEUR_DUPONT, AUTEUR_LEROY, Optional.of(LE_10_MAI_2026_A_7H)));
  ...
  void shouldNotOuvrirDeuxJourneesEnMemeTemps() {
    service.arrive(new ArriveeAEnregistrer(OPERATEUR_DUPONT, AUTEUR_DUPONT));
    ArriveeAEnregistrer seconde = new ArriveeAEnregistrer(OPERATEUR_DUPONT, AUTEUR_DUPONT);
  ...
  void shouldPointerPauseRepriseEtDepartSurLaJourneeEnCours() {
    ...
    service.pointe(new PointageDePresenceAEnregistrer(OPERATEUR_DUPONT, AUTEUR_DUPONT, TypeDEvenementDePresence.PAUSE));
    maintenant.set(LE_10_MAI_2026_A_13H);
    service.pointe(new PointageDePresenceAEnregistrer(OPERATEUR_DUPONT, AUTEUR_DUPONT, TypeDEvenementDePresence.REPRISE));
    maintenant.set(LE_10_MAI_2026_A_17H);
    JourneeDeTravail journee = service.pointe(
      new PointageDePresenceAEnregistrer(OPERATEUR_DUPONT, AUTEUR_DUPONT, TypeDEvenementDePresence.DEPART)
    );
  ...
  void shouldRegulariserUnDepartOublie() {
    JourneeDeTravail ouverte = service.arrive(new ArriveeAEnregistrer(OPERATEUR_DUPONT, AUTEUR_DUPONT));
    maintenant.set(LE_11_MAI_2026_A_9H15);
  ...
  void shouldAnnulerUnPointageEnTrop() {
    ...
    JourneeDeTravail avecPause = service.pointe(
      new PointageDePresenceAEnregistrer(OPERATEUR_DUPONT, AUTEUR_DUPONT, TypeDEvenementDePresence.PAUSE)
    );
  ...
  void shouldListerLesJourneesDUnOperateurSurUnePeriode() {
    ...
    assertThat(service.list(Optional.of(journeeDu10Mai2026()), Optional.of(OPERATEUR_DUPONT), firstPageOfTen()).content()).containsExactly(
      journee
    );
  }
}
