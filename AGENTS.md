...
- **OpenAPI** : springdoc est au classpath, la configuration globale vit dans `wire/openapi`. Seul `atelier` est annoté (`@Tag`, `@Operation`, `@ApiResponse`, `@Schema`) — c'est le patron à suivre pour les contextes suivants, `elementdefabrication` restant à reprendre.
  - **`documentation/openapi.json` est un livrable commité** — ce qu'il vaut pour le front est dans [documentation/atelier-api.md](documentation/atelier-api.md). Ici, ce qui coûte : `OpenApiConfigurationIT` le compare à la spec servie, donc toute signature qui bouge dans un `*Resource.java` casse le build tant qu'on n'a pas régénéré par `mvn --batch-mode -ntp verify -Dopenapi.regenerate=true` — la seule chose autorisée à le réécrire. Un `@RestController` de test doit porter `@Hidden`, sinon il entre dans le livrable. Et la mise en page appartient au générateur, d'où sa ligne dans `.prettierignore`.
