...
Ce fichier fournit aux agents les règles à respecter en permanence sur ce projet.
...
**Chaque bounded context porte son propre `AGENTS.md`**, à la racine de son package (`src/main/java/com/glm/glmback/<contexte>/AGENTS.md`) : il énonce ce dont le contexte s'occupe, ce dont il ne s'occupe **pas**, ses agrégats, ses invariants et ses ports sortants. Le lire avant de toucher au contexte, et le créer avec tout nouveau bounded context. Il ne duplique ni les règles de code ci-dessous, ni la justification métier de `documentation/contexte-metier.md` — il pointe vers elles.
...
- **OpenAPI** : springdoc est au classpath, la configuration globale vit dans `wire/openapi`. Seul `atelier` est annoté (`@Tag`, `@Operation`, `@ApiResponse`, `@Schema`) — c'est le patron à suivre pour les contextes suivants, `elementdefabrication` restant à reprendre.
  - **`documentation/openapi.json` est un livrable commité, régénéré automatiquement** — ce qu'il vaut pour le front est dans [documentation/atelier-api.md](documentation/atelier-api.md). `mvn verify` (donc aussi `mvn install`) le réécrit systématiquement : `exec-maven-plugin` (phase `pre-integration-test`, cf. `pom.xml`) exécute `OpenApiSpecificationGenerator` (`wire/openapi/infrastructure/primary`, JVM séparée sur le classpath de test) — ce n'est pas un test, juste un `main()` — qui démarre la vraie application (`GlmprojectApp` + `TestSecurityConfiguration`, profil `test`, port aléatoire), appelle `/v3/api-docs`, canonicalise la réponse (clés triées, indentation fixe, `servers` retiré) et écrase le fichier, avant tout IT ou scénario Cucumber. Il n'y a plus ni commande ni propriété système à activer. Un `@RestController` de test doit porter `@Hidden`, sinon il entre dans le livrable. Et la mise en page appartient au générateur, d'où sa ligne dans `.prettierignore`.
  - **Régénérer fait partie de la modification, pas un commit à part** : toute création ou modification d'un endpoint (nouvelle route, champ ajouté/retiré/renommé sur un DTO `Rest*`, statut ou tag OpenAPI qui change) régénère `documentation/openapi.json` avant de committer, dans le même commit que le code — lancer `mvn --batch-mode -ntp verify` (Docker requis) suffit, puis relire `git diff documentation/openapi.json`. Oublier de committer le résultat est rattrapé par la CI, qui échoue sur `git diff --exit-code -- documentation/openapi.json` après le `mvn verify` (voir `.github/workflows/github-actions.yml`).
- **Erreurs métier** : une exception traduite par un `@ControllerAdvice` sort en `ProblemDetail` dont le `type` porte un code stable `urn:glm:erreur:<contexte>:<code>`, dérivé d'une constante d'`enum` par bounded context implémentant `ProblemCode` (`shared/error/infrastructure/primary`). **Le nom de la constante est le contrat publié** — le renommer change ce que reçoit le front, sans que rien d'autre ne bronche. Ces codes n'entrent pas dans `openapi.json` : ce sont les `*ExceptionAdviceTest` qui les tiennent, chacun avec un test d'exhaustivité qui échoue dès qu'un handler n'a pas sa ligne de table. Voir [documentation/codes-erreur.md](documentation/codes-erreur.md).
