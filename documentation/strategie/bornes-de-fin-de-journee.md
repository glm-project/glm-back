...
| D11 | Le gestionnaire dispose d'une **liste des anomalies** : journées abandonnées sans départ, journées fermées dont l'amplitude dépasse le seuil, pointages signalés ou mis en attente (D12).                                                                                                                                                                                                                    | Sans notification, personne ne consultera peut-être la liste.                                                                                               |
| D12 | **L'opérateur n'est jamais empêché de pointer ni de commencer son activité.** Aucun pointage ne rend d'erreur : chaque geste est enregistré dès qu'il peut l'être, signalé plutôt que mis en attente (tableau ci-dessous). **Seule exception : un OF clôturé**, qui n'est plus pointable ; l'opérateur en est informé par un message. Les actes du gestionnaire, eux, peuvent être refusés avec explication. | Un pointage signalé compte immédiatement, avant que le gestionnaire l'ait vu. Et le pupitre ne peut plus signaler à l'opérateur une erreur de manipulation. |

### Aucun pointage ne rend d'erreur

Un pointage est un geste de l'opérateur : arrivée, pause, reprise, départ, démarrer, non-conformité, arrêter. Le pupitre reçoit toujours un succès, sauf sur un OF clôturé (voir le tableau). Le serveur choisit l'une de ces issues, en préférant toujours celle qui fait compter le temps :
...
- **mis en attente** : réservé aux gestes qu'on ne peut rattacher à rien (opérateur, poste ou OF inconnu du serveur, geste corrompu). Le geste est conservé tel quel, hors de tous les calculs, et apparaît dans la liste des anomalies ; le gestionnaire l'applique par une régularisation, ou l'écarte.

![Aucun pointage ne rend d'erreur](bornes-de-fin-de-journee/12-aucun-pointage-refuse.svg)
...
| Geste de présence rejoué dans le désordre qui casse l'enchaînement  | `transition-de-presence-interdite`                                                     | deux pupitres, l'un hors ligne                             | **absorbé** s'il est redondant, sinon **mis en attente**                                                |
...
| Opérateur, poste ou OF inconnu du serveur                           | `operateur-introuvable`, `poste-de-travail-introuvable`, `suivi-d-atelier-introuvable` | référentiel du pupitre périmé, fiche supprimée entre-temps | **mis en attente**                                                                                      |
...
| Même identifiant de geste, contenu différent                        | `identifiant-evenement-reutilise`                                                      | défaut du pupitre                                          | **mis en attente**, sous un identifiant attribué par le serveur                                         |

Un OF clôturé est la seule exception : il n'est plus visible au pupitre, et un pointage qui l'atteint encore (pupitre hors ligne pendant la clôture) est refusé avec un message qui l'explique à l'opérateur. Arrêter une activité sur cet OF reste absorbé, puisque la clôture l'a déjà arrêtée.
...
| 8   | Aucun pointage refusé : absorption, redressement, mise en attente             | tableau des refus (§2), E5 | 6         |
...
- **Règles :**
  ...
  - trois types : « journée sans départ » (abandonnée), « amplitude excessive » (journée fermée au-delà du seuil) et « pointage signalé ou en attente » (D12, ajouté par le lot 8) ;
...
- **Règles :**
  ...
  - un pointage signalé compte immédiatement et apparaît dans les anomalies ;
  - un pointage mis en attente est conservé tel quel, hors des calculs ; le gestionnaire l'applique par une régularisation, ou l'écarte ;
  - le pupitre n'affiche plus qu'un seul refus : « OF clôturé, vous ne pouvez plus pointer dessus ».
- **Acceptation :** chaque ligne du tableau devient un scénario, qui vérifie une réponse positive et l'issue attendue (le refus explicite pour l'OF clôturé) ; un pointage mis en attente apparaît dans les anomalies, puis en disparaît une fois appliqué ou écarté.
...
5. Un pointage mis en attente : faut-il en informer l'opérateur au pupitre (« pointage enregistré, à valider »), ou seulement le gestionnaire ?
6. Que peut faire le gestionnaire d'un pointage mis en attente : l'appliquer tel quel, l'appliquer corrigé, l'écarter ? Et faut-il un motif pour l'écarter ?
