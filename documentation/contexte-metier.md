...
**L'habilitation est une règle dure pour le gestionnaire, un signalement pour le pupitre.** Une régularisation ou une correction sur un poste où l'opérateur n'est pas déclaré est refusée (409), par `Habilitations`. Un pointage du pupitre sur un tel poste est en revanche enregistré et **signalé** au gestionnaire (lot 8b) : l'habilitation a pu être retirée pendant que le pupitre était hors ligne, et le temps de l'opérateur compte. La règle ne joue que lorsqu'un poste est fourni : une entreprise sans parc machine n'a aucune habilitation à déclarer et retrouve son comportement nominal.

**Les pointages signalés** (`GET /api/atelier/pointages-signales`, gestionnaire seul) sont les gestes du pupitre enregistrés malgré tout : `OPERATEUR_NON_HABILITE`, `DATE_ANTERIEURE_A_L_ENGAGEMENT` (ramené à l'engagement) et `DATE_FUTURE` (ramené à la réception), cumulables, avec la date envoyée quand elle a été ramenée. Contrairement aux anomalies, ils sont **stockés** — un fait passé ne se déduit pas du journal une fois l'événement redressé — dans la table `pointage_signale`, un par événement. Ils sortent de la liste quand l'événement est annulé, corrigé, ou acquitté par le gestionnaire ; l'opérateur n'en est pas informé.

**Les pointages en attente** (`GET /api/atelier/pointages-en-attente`, gestionnaire seul) sont les gestes du pupitre qu'on ne sait rattacher à rien (lot 8c) : opérateur, poste ou élément inconnu, geste rejoué dans le désordre ou daté dans une journée fermée, identifiant réutilisé avec un autre contenu. Le pupitre reçoit un succès (`202`) ; le geste est conservé tel quel dans la table `pointage_en_attente`, **hors de tous les calculs**. Le gestionnaire l'**applique** — une régularisation à sa date, refusable avec explication, corrigeable ensuite — ou l'**écarte**, motif obligatoire. La décision de mettre en attente appartient au domaine (`PointagesEnAttenteService.recueille`), qui ne retient que les refus disant « rattaché à rien » ; ces refus sont tous levés avant la moindre écriture. Un élément clôturé reste le seul refus montré à l'opérateur.
...
2. **La quarantaine des gestes refusés est faite côté serveur** (lot 8 de
   [strategie/bornes-de-fin-de-journee.md](strategie/bornes-de-fin-de-journee.md)) : une habilitation retirée est
   enregistrée et signalée, les gestes rattachés à rien sont mis en attente, et un suivi clôturé reste le seul refus
   montré à l'opérateur. Reste au front de retirer de sa politique de rejeu tous les autres refus.
